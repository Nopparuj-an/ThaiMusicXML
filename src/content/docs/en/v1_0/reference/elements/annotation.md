...
Either plain text or one [`<text>`](/en/v1_0/reference/elements/text/) child per alignment. The `<text>` children are optional, but each alignment can appear at most once.
...
An annotation renders where its position puts it: in the title band, between two section blocks, or above one part's first row. See [Annotations](/en/v1_0/reference/rendering/#annotations).
