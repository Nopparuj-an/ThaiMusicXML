...
Either plain text or up to three [`<text>`](/en/v1_0/reference/elements/text/) children, one per alignment, the same content model [`<annotation>`](/en/v1_0/reference/elements/annotation/#content) uses. `align` is required on each and accepts `left`, `center`, or `right`, and each alignment may appear at most once.
...
- This names the writer only. The words themselves go in a [lyric part](/en/v1_0/reference/elements/part/#part-types), as [`<syllable>`](/en/v1_0/reference/elements/syllable/) elements on the grid.
- Renders in the title band. See [The title band](/en/v1_0/reference/rendering/#the-title-band).
