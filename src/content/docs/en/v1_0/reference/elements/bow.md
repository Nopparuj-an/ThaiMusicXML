...
The `<bow>` element marks a span of sibling [`<note>`](/en/v1_0/reference/elements/note/) elements played with one continuous bow stroke on a stringed instrument, without changing direction across the span.
...
A span that crosses a `<line>` boundary is still one span. The renderer splits the curve in two and marks the cut, which changes nothing about the data. See [Bow spans across a line break](/en/v1_0/reference/rendering/#bow-spans-across-a-line-break).
