...
One bpm beat is **two** measure beats, meaning two consecutive `<note>`, `<rest>`, or `<group>` slots as defined under [`<measure>`](/en/v1_0/reference/elements/measure/#beats). At `<bpm>90</bpm>` the score plays 180 note slots a minute.
...
Attacks fall a uniform slot apart unless a [`<group>`](/en/v1_0/reference/elements/group/) intervenes, so a player walks the slots at that interval and subdivides each group backwards inside its own slot. Beat `n` of a measure arrives `n` slots from the measure's leading edge, which puts the measure's last note on its closing boundary. See [Where the children fall](/en/v1_0/reference/elements/group/#where-the-children-fall).
