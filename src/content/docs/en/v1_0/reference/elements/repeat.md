...
- [`<repeat>`](/en/v1_0/reference/elements/repeat/) - a repeat may wrap another repeat
...
`<repeat>` does not change how section order is read. The order is still the document order of the `<section>` elements, walked depth first through any `<repeat>` wrappers. See [`<structure>`](/en/v1_0/reference/elements/structure/#section-order).
