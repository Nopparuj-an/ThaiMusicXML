...
The [Hello World](/en/v0_1/tutorial/1-hello_world/) example used one instrument and one section. This page adds a second instrument, performance directions, annotations, and a repeat, using the real ลาวดวงเดือน (Lao Duang Duen).
...
- **`<direction>`**: Performance directions that apply to the whole score.
  ...
  - `<chan>` sets the ชั้น (rhythmic layer). `value="2"` means สองชั้น. ชั้น is not tempo; it is the spacing of the ฉิ่ง cycle. See [The score model](/en/v0_1/tutorial/3-the_score_model/) for the distinction.
...
A section's order comes from its position in `<structure>`, not from an attribute. Repeating a section does not change line or measure numbers. The second playthrough restarts at line 1, measure 1. See [Sections, repeats, and endings](/en/v0_1/tutorial/6-sections_repeats_endings/) for nested repeats and `<line-repeat>`.
...
Next, see [The score model](/en/v0_1/tutorial/3-the_score_model/) for the conceptual foundations that make these elements work the way they do.

:::note[Elements introduced]
[`<composer>`](/en/v0_1/reference/elements/composer/), [`<direction>`](/en/v0_1/reference/elements/direction/), [`<nathap>`](/en/v0_1/reference/elements/nathap/), [`<chan>`](/en/v0_1/reference/elements/chan/), [`<bpm>`](/en/v0_1/reference/elements/bpm/), [`<repeat>`](/en/v0_1/reference/elements/repeat/), [`<annotation>`](/en/v0_1/reference/elements/annotation/), [`<text>`](/en/v0_1/reference/elements/text/), [`<instrument-short-name>`](/en/v0_1/reference/elements/instrument-short-name/)
:::
