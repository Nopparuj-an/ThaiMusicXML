...
A beat arrives on its last slot, so a measure is read as travelling toward its final note rather than starting from its first, and that final note lands on the measure's closing boundary. This governs where a [`<group>`](/en/v0_1/reference/elements/group/#where-the-children-fall)'s children fall and where every symbol sits in the cell.
