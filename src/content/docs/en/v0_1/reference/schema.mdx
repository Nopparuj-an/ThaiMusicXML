...
description: The RELAX NG schema for ThaiMusicXML v0.1 and the conformance corpus that goes with it
---
...
<a href="/schema/thaimusicxml-0.1.rng"><code>/schema/thaimusicxml-0.1.rng</code></a> is the schema. It is RELAX NG in XML syntax, so `xmllint` and anything else built on libxml2 reads it without conversion:

```sh
xmllint --noout --relaxng thaimusicxml-0.1.rng score.txml
```

<a href="/schema/thaimusicxml-0.1.xsd"><code>/schema/thaimusicxml-0.1.xsd</code></a> is the same schema translated to W3C XML Schema, for tools that read XSD and not RELAX NG. It's generated from the `.rng` file by [Trang](https://relaxng.org/jclark/trang.html) on every push; the RELAX NG file is the one actually hand-edited, so treat the XSD as downstream of it.
...
So conformance has two layers, and passing the schema is the first of them, not the whole of it. The [Conformance reference](/en/v0_1/reference/conformance/) states every rule; `scripts/check-corpus.mjs` in the repository implements the ones the grammar cannot express, and runs both layers over the corpus.
